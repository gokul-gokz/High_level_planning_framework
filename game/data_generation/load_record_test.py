                    ...
                    if i < len(plan):
                        ...
                        for k in all_sprites:
                            ...
                            if plan[i][3][j*3] != plan[i][0][j*3] or plan[i][3][j*3+1] != plan[i][0][j*3+1] or plan[i][3][j*3+2] != plan[i][0][j*3+2]:
                                k.surf.fill((255, 0, 0))
                                x = plan[i][0]
                                s_str = '('+ str(int(x[j*3]*1000)) + ',' + str(int(x[j*3+1]*1000)) + ',' + str(x[j*3+2]) + ')'
                                s.append(font.render(s_str, True, (255, 0, 0)))
                                x = plan[i][3]
                                s_prime_str = '('+ str(int(x[j*3]*1000)) + ',' + str(int(x[j*3+1]*1000)) + ',' + str(x[j*3+2])  + ')'
                            ...
                            k.update(plan[i][3][j*3]/2*100+500, plan[i][3][j*3+1]/2*100+500)
                            j+=1
